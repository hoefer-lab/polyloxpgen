...
def test_single_sample_output_bc1022_ld2017():
    purged_barcodes_ref = np.loadtxt('./original/bc1022_BarPipe.txt', usecols=0, skiprows=1, delimiter='\t', dtype=str)
    purged_reads_ref = np.loadtxt('./original/bc1022_BarPipe.txt', usecols=1, skiprows=1, delimiter='\t')
    data_minrecs_ref = np.loadtxt('./original/bc1022_BarPipe.txt', usecols=3, skiprows=1, delimiter='\t', dtype=int)
    data_pgen_ref = np.loadtxt('./original/bc1022_BarPipe.txt', usecols=4, skiprows=1, delimiter='\t')

    # run polylox_merge
    df_merged = polyloxpgen.merge.polylox_merge(['./original/bc1022.barcode.count.txt'],
                                    ['bc1022'], './temp/', 'bc1022_merged')

    # run polylox_pgen
    df_pgen = polyloxpgen.pgen.polylox_pgen('./temp/bc1022_merged.txt',
                                        './temp/', 'bc1022_pgen',
                                        path_matrix_type='ld_2017')

    # read back from resulted file (corresponds to df_pgen)
    df_res = pd.read_csv('./temp/bc1022_pgen.txt', sep='\t', index_col=0)

    # delete temporary files in the end
    if os.path.isfile('./temp/bc1022_merged.txt'):
        os.remove('./temp/bc1022_merged.txt')
    if os.path.isfile('./temp/bc1022_pgen.txt'):
        os.remove('./temp/bc1022_pgen.txt')
...
def test_two_sample_output_bc1020_bc1022_uniform():
    ...
    df_merged_ref = pd.read_csv('./original/bc1020_bc1022_merged.txt', sep='\t', index_col=0)
    df_pgen_ref = pd.read_csv('./original/bc1020_bc1022_pgen_uniform.txt', sep='\t', index_col=0)

    # run polylox_merge
    df_merged = polyloxpgen.merge.polylox_merge(['./original/bc1020.barcode.count.txt',
                                                './original/bc1022.barcode.count.txt'],
                                    ['bc1020', 'bc1022'], './temp/', 'bc1020_bc1022_merged')

    # run polylox_pgen
    df_pgen = polyloxpgen.pgen.polylox_pgen('./temp/bc1020_bc1022_merged.txt',
                                        './temp/', 'bc1020_bc1022_pgen',
                                        path_matrix_type='uniform')

    # read back from resulted files (correspond to df_merged and df_pgen)
    df_merged_res = pd.read_csv('./temp/bc1020_bc1022_merged.txt', sep='\t', index_col=0)
    df_pgen_res = pd.read_csv('./temp/bc1020_bc1022_pgen.txt', sep='\t', index_col=0)

    # delete temporary files in the end
    if os.path.isfile('./temp/bc1020_bc1022_merged.txt'):
        os.remove('./temp/bc1020_bc1022_merged.txt')
    if os.path.isfile('./temp/bc1020_bc1022_pgen.txt'):
        os.remove('./temp/bc1020_bc1022_pgen.txt')
